...
namespace LeagueSharp.Common
{
    ...
    public static class Prediction
    {
        ...
        static Prediction()
        {
            ...
            menu.AddItem(new MenuItem("predictionmethod", "Prediction Method").SetShared().SetValue(new StringList(new[] { "Old Prediction", "Updated Prediction (Best)", "Updated Prediction (Decent)", "BadaoPrediction" }))).ValueChanged +=
                (sender, args) =>
                {
        ...
        internal static PredictionOutput GetPrediction(PredictionInput input, bool ft, bool checkCollision, int method = 0)
        {
            ...
            if (result == null)
            {
                switch (method)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        result = GetBadaoPrediction(input);
                        break;
            ...
            if (Math.Abs(input.Range - float.MaxValue) > float.Epsilon)
            {
                ...
                if (result.Hitchance >= HitChance.High &&
                    input.RangeCheckFrom.Distance(result.UnitPosition, true) >
        ...
        internal static PredictionOutput GetBadaoPrediction(PredictionInput input)
        {
            if (input.Unit.IsMoving)
            {
                Vector3 chuot = GetPositionOnPath(input,input.Unit.GetWaypoints(),input.Unit.MoveSpeed).UnitPosition;//(input.Unit.Position.To2D() + input.Unit.MoveSpeed * input.Delay * input.Unit.Direction.To2D().Perpendicular()).To3D();//Prediction.GetPrediction(input.Unit, 100).CastPosition;
                float dis = input.From.Distance(input.Unit.Position);
                float rad = input.Unit.BoundingRadius + input.Radius / 2 - 50 > 100 ? 100 : input.Unit.BoundingRadius + input.Radius / 2 - 50;
                double x = math.t(input.Unit.MoveSpeed, input.Speed, dis, input.Delay + Game.Ping / 1000, rad, input.From, input.Unit.Position, chuot);
                var castpos = input.Unit.Position.Extend(chuot, (float)x * input.Unit.MoveSpeed - rad);
                if (x != 0 && !input.Unit.IsDashing() && input.RangeCheckFrom.Distance(castpos) <= input.Range)
                {
                    return new PredictionOutput()
                    {
                        CastPosition = castpos,
                        UnitPosition = (input.Unit.Position.To2D() + input.Unit.MoveSpeed * (float)x * input.Unit.Direction.To2D().Perpendicular()).To3D(),
                ...
                {
                    rad = 1;
                    x = math.t(input.Unit.MoveSpeed, input.Speed, dis, input.Delay + Game.Ping / 1000, rad, input.From, input.Unit.Position, chuot);
                    castpos = input.Unit.Position.Extend(chuot, (float)x * input.Unit.MoveSpeed - rad);
                    if (x != 0)
                    {
                        return new PredictionOutput()
                        {
                    ...
                    else return new PredictionOutput()
                    {
                        CastPosition = input.Unit.ServerPosition,
                        UnitPosition = (input.Unit.Position.To2D() + input.Unit.MoveSpeed * (float)x * input.Unit.Direction.To2D().Perpendicular()).To3D(),
